/*Core Windows text editor code base - Foundation module for custom
  ...
  Check various mouse 16/32 bit parts
  Check HIWORDs and LOWORDs
  Check for numeric overflows
  Check unsigned vs. signed integer areas
*/
...
static int caretLnOffst; /* Difference from text line to displayed line (used to emulate weird caret snapping behavior) */
...
static bool caretVisible; /* Cleared if the caret is scrolled out of view */
static bool overwrite; /* Is overwrite mode enabled? */
static bool wasInsert; /* Was the last operation an insert or a delete? */
static bool sameUndoOp; /* Should the next operation be added as part of the current undo entry? */
...
static unsigned* lineStarts; /* This array always maintains an extra entry that is equal to the text size. */
...
static BOOL w95Wheel = FALSE; /* Has Windows 95 mouse wheel support been initialized? */
...
static bool mBtnTimeUp; /* Will releasing the middle mouse button deactivate the mouse pan mark? */
...
/* Render parameters */
static bool riInitialized; /* Set to true upon first call to UpdateRenderInfo() */
static unsigned beginUpdLine, endUpdLine; /* Line range for recalculating render info (display relative) */
/* End update line is one line beyond the lines getting updated */
static unsigned norUpLine, numNorUpLines; /* Offset from and number in visible text lines to redraw */
//static unsigned regUpLine, numRegUpLines; /* Offset from and number in visible region lines to redraw */
static unsigned lastWrappedLine; /* Which line was last corrected in rewrapping or retruncating lines */
static unsigned lastTextPos; /* Used to find which parts of the region need to be drawn */
static unsigned lastCaretLine; /* Same as lastTextPos */
static RECT updRect; /* Used to verify that the update region has not changed since InvalidateRect() */
...
static unsigned totVisLines; /* Maximum partial visible lines that will fit in the window */
static unsigned regBeginLine; /* Which line the region begins on */
static unsigned regVisBegLn; /* The first visible region line (display relative) */
...
static unsigned visLnOffset; /* Offset from top of first partial visible line to window top */
...
static unsigned* firstVisChars; /* First characters on each line to be partially visible */
static unsigned* firstVisOffset; /* Offset from beginning of partially visible characters to left window edge */
static POINT lastMousePos;
static POINT scrdiff; /* Difference from client area origin to screen origin */
...
void GenTabStops();
void WrapWords(bool rewrap/* = false*/, unsigned hint1, unsigned hint2);
void ProcessTabs(unsigned lineNum, int* charWidths, unsigned endIndex);
void TruncateLines();
void UpdateRenderInfo(bool onlyRegion/* = false*/, bool reindex/* = false*/);
...
void MScrollEnd();
void MWheelScroll(short amount);
...
void InsertLineStart(unsigned line);
void DeleteLineStarts(unsigned begin, unsigned end);
void CopyRegion();
...
kind of boring. aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa                        aaaa\n\tNow you should make sure new line handling is O.K.");*/
	...
	case WM_PAINT:
	{
			...
			caretLine < visLine + endUpdLine)
		{
			...
			HideCaret(hwnd);
			rt.left = 0;
			rt.top = (int)bordWidthY + (caretLine - visLine) * fontHeight - visLnOffset;
			rt.right = winWidth;
			rt.bottom = (int)bordWidthY + (caretLine + 1 - visLine) * fontHeight - visLnOffset;
			hDC = GetDC(hwnd);
			hMemDC = CreateCompatibleDC(hDC);
			hBmCaret = CreateCompatibleBitmap(hDC, rt.right, rt.bottom - rt.top);
			...
			ExtTextOut(hMemDC, bordWidthX + leftMargin, 0, ETO_OPAQUE, &rt2,
			...
			if (regionActive == true && regVisBegLn + numRegVisLines > caretLine - visLine)
			{
				ExtTextOut(hMemDC, bordWidthX + leftMargin, 0, ETO_OPAQUE, &rt2,
					norTextRI[cvl].lpstr, norTextRI[cvl].n, norTextRI[cvl].pdx);
			}
			...
			BitBlt(hDC, 0, rt.top, rt.right, rt.bottom - rt.top, hMemDC, 0, 0, SRCCOPY);
...
/* Wraps or rewraps words in the buffer.
   ...
   "hint1" and "hint2" specify the points in ascending order that text
   was deleted in. If the shared variable "wasInsert" is false, then
   these variables are ignored. */
void WrapWords(bool rewrap, unsigned hint1, unsigned hint2)
{
	unsigned curLine;
	unsigned wrapPos;
	bool foundNewline;
	...
	if (rewrap == true)
	{
		...
		if (wasInsert == false)
		{
			/* Remove invalid line beginning references */
			for (i = caretLine; i < numLines; )
			{
				if (lineStarts[i] >= textSize && numLines != 1)
				{
					/* Delete this line start */
					DeleteLineStarts(i, i + 1);
					deletedLines = true;
					if (i == caretLine && curLine > 0)
					{
						/* Start wrapping words one before the caret line */
						curLine--;
						wrapPos = lineStarts[curLine];
	...
	while (wrapPos < textSize)
	{
		...
		if (rewrap == true && wasInsert == true)
		{
			if (lineStarts[curLine+1] != 0 &&
				lineStarts[curLine+1] > textPos &&
				buffer[lineStarts[curLine+1]-1] == '\n')
			{
				/* Invalidate all subsequent lines */
				foundNewline = true;
			}
		}
		...
		/*if (wrapPos + 1 < textSize && buffer[wrapPos+1] == ' ')
		{
			//Break at this position because the width does not count
			wrapPos += 2;
		}
		else */if (lastSpace != 0 && (buffer[lastSpace] == ' ' ||
		...
		if (wasInsert == false && buffer[wrapPos-1] != '\n' &&
			wrapRes.nMaxFit == wrapStride && newLength < textAreaWidth)
		{
			/* Delete any trailing line starts */
			DeleteLineStarts(curLine + 1, numLines);
			deletedLines = true;
			break;
		}
		...
		if (rewrap == true)
		{
			if (wrapPos > textSize)
				break;
			if (wrapPos > textPos && lineStarts[curLine+1] == wrapPos/* &&
				wrapPos - lineStarts[curLine] == wrapRes.nMaxFit &&
				newLength < textAreaWidth*/)
			{
				/* Stop rewrapping if no change is necessary */
				break;
			}
			if (wrapPos > textPos && buffer[lastSpace] == '\n')
			{
				if (wasInsert == false)
				{
					/* Search forward for the last line start reference that
					   ends with a newline, then delete the line end
					   references in between */
					unsigned lastNewline;
					for (lastNewline = curLine + 1;
						buffer[lineStarts[lastNewline]-1] != '\n';
						lastNewline++);
					if (lastNewline > curLine + 1)
					{
						DeleteLineStarts(curLine + 1, lastNewline);
						deletedLines = true;
					}
				}
				/* Otherwise, don't process already existing newlines */
				break;
			}
...
void UpdateRenderInfo(bool onlyRegion, bool reindex)
{
	char* curLinePtr;
	unsigned lineSize;
	...
	/* We must always update line references for preceding lines, even */
	/* if the character content wasn't modified */
	...
	for (i = beginLine; i < endLine; i++)
	{
		...
		else
		{
			...
			if (firstVisChars[i] > curLineStart - lineStarts[visLine+i])
				curLinePtr = &buffer[lineStarts[visLine+i]+firstVisChars[i]];
			else
			{
				lineSize -= ((curLineStart - lineStarts[visLine+i]) -
					firstVisChars[i]);
			}
		}
		...
		regTextRI[i-beginLine].y = (int)bordWidthY - visLnOffset + fontHeight * i;
		...
		regTextRI[i-beginLine].uiFlags = ETO_OPAQUE;
			...
			regTextRI[i-beginLine].rcl.right = regTextRI[i-beginLine].x + curLineWidth;
			regTextRI[i-beginLine].rcl.bottom = regTextRI[i-beginLine].y + fontHeight;
...
void InsertText(char c, const char* str, bool silent)
{
	if (c != 0)
	{
			...
			buffer[textPos] == '\0')
		{
			...
			for (i = caretLine + 1; i < numLines; i++)
				lineStarts[i]++;
		}
		buffer[textPos] = c;
		textPos++;
...
void DeleteText(unsigned p1, unsigned p2, bool silent)
{
	...
	lastNumLines = numLines;
	{
		...
		{
			if (p2 - 1 < lineStarts[p2Line])
				break;
		}
		p2Line--;
		lineStarts[p2Line+1] -= (p2 - p1);
		for (curLine = p1Line; curLine <= p2Line; curLine++)
		{
			if (lineStarts[curLine] > p1 && lineStarts[curLine] < p2 &&
				buffer[lineStarts[curLine]-1] == '\n' && numLines > 1)
			{
				DeleteLineStarts(curLine, curLine + 1);
				curLine--;
				if (curLine < p2Line)
					p2Line--; /* Make sure the line index stays correct */
				deletedLines = true;
			}
		}
	}
	...
	/* Correct line beginning indices after the deleted text */
	{
		unsigned i;
		unsigned p2Line;
		for (p2Line = caretLine; p2Line < numLines; p2Line++)
		{
			if (p2 - 1 < lineStarts[p2Line])
				break;
		}
		p2Line--;
		for (i = p2Line + 1; i < numLines; i++)
			lineStarts[i] -= (p2 - p1);
	}
	...
	/* Update the window */
	if (lastNumLines > totVisLines && lastNumLines > numLines &&
		...
		wParam != VK_DELETE)
	{
		...
		case VK_UP: /* Scroll view one line up */
		{
			if (caretLine == 0)
				break;
			//lastMousePos.x = bordWidthX + leftMargin + caretX;
			//lastMousePos.y = (int)bordWidthY + fontHeight * (caretLine - 1 - visLine) - visLnOffset;
			//TextPosFromPoint();
...
void CalcCaretPos()
{
	...
	else
	{
		...
		/* Temporary variables */
		unsigned i;
		curRI = &norTextRI[caretLine-visLine];
		while (firstVisChars[caretLine-visLine] > textPos)
			ScrollContents(true, 0, -(int)avgCharWidth, SB_HORZ);
		while (firstVisChars[caretLine-visLine] +
			   numVisChars[caretLine-visLine] >= textPos)
			ScrollContents(true, 0, avgCharWidth, SB_HORZ);
		for (i = firstVisChars[caretLine-visLine]; i < textPos; i++)
			caretX += curRI->pdx[i-firstVisChars[caretLine-visLine]];
...
void UpdateRegion(bool setRegActive)
{
	...
	if (regionActive == true)
	{
		...
		lastTextPos = textPos;
		lastCaretLine = caretLine;
		SendMessage(GetParent(lastHwnd), NM_REGACTIVE, 0, (LPARAM)TRUE);
	}
	else
	{
		RECT rt;
		...
		CopyMemory(&updRect, &rt, sizeof(RECT));
		numRegVisLines = 0;
		SendMessage(GetParent(lastHwnd), NM_REGACTIVE, 0, (LPARAM)FALSE);
	}
}
